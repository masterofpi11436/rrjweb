...
@section('content')
    ...
    <form method="GET" action="{{ route('public.reports.calendar-year-graph') }}" class="mb-6 flex gap-4 items-end">
        <div>
    ...
    <div class="mb-8" style="overflow-x: auto;">
        <div style="height: 320px;">
            <canvas id="itemYearTrendChart" style="width: 100%; height: 100%;"></canvas>
        </div>
    </div>
...
window.addEventListener('load', function () {
    // ====== Monthly Trend (line) ======
    const trendCtx = document.getElementById('itemYearTrendChart').getContext('2d');
    new Chart(trendCtx, {
        type: 'line',
        data: {
            labels: {!! json_encode($monthLabels) !!},
            datasets: [{
                label: 'Total Quantity per Month',
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Monthly Trend for "{{ $itemName }}" in {{ $selectedYear }}',
                    font: { size: 18 }
                },
